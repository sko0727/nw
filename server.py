...
def handler(client, address):          ## ()안에 client, address를 넣어줘서 client, address 인자를 받아옴.      
    while True:                                 
        try:                       
            # 클라이언트가 전송한 데이터 수신
...
# 서버 정보
# * - 모든이라는 뜻을 가지고 있음 컴퓨터에서(와일드카드라고 한다고함)
# 127.0.0.1의 ip는 자기 자신의 ip 주소 
info = ("0.0.0.0", 9999)
# 소켓 생성
s = socket.socket()
...
while True:                                     # while 문으로 인해 서버가 계속 실행된다.
    client, address = s.accept()                # 필요한 접속인지 아닌지 확인, s.accept가 새로운 접속을 처리함.
